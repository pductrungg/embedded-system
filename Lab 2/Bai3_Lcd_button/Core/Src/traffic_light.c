...
// Light durations
uint8_t GREEN_DURATION = 5;
uint8_t YELLOW_DURATION = 2;
uint8_t RED_DURATION = 7;
...
uint8_t GREEN_COUNTER = 5;
uint8_t YELLOW_COUNTER = 2;
uint8_t RED_COUNTER = 7;
...
//    counter_blink =(counter_blink+1)%10;
    if (counter == 0) {
        if (systemState == NORMAL) {
            stateCounter--;
            if (GREEN_COUNTER > 0) GREEN_COUNTER--;
			if (YELLOW_COUNTER > 0) YELLOW_COUNTER--;
			if (RED_COUNTER > 0) RED_COUNTER--;
